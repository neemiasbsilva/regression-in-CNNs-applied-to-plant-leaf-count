...
Original file is located at
    https://colab.research.google.com/drive/15rX4hyK3w64b1TA1ca5TAjx5BG99QCwA
...
"""# Implementation Convolution Neural Network with Inception ResNET V2"""
...
"""**Implementation about RMSprop (InceptionResNetV2)**"""
...
#model.save('/content/drive/My Drive/ColabNotebooks/AllmodeloRMSprop.h5')
model.save('/content/drive/My Drive/ColabNotebooks/Inception/modelInception.h5')

"""# Carregar o Modelo Trainado"""
...
"""# Predict to Inception Res Net V2 aplied to RMSprop
...
"""# Metrics About InceptionResNetV2

**R², MAE and MSE for Set of Train**
"""
...
"""**R², MAE and MSE for Set of Validation**"""
...
"""# Implementação do Deslizamento na Imagem"""

square_size = 50
heatmap = np.zeros(((width-square_size+1)/2, (height-square_size+1)/2))
print(heatmap.shape)
for i in range(0,width-square_size,20):
    for j in range(0,height-square_size,20):
        print((i,j))
        x_copy = []
        x_copy.append(np.copy(x_test[0,:]))
        x_copy = np.asarray(x_copy)
        print(x_copy.shape)
        x_copy[0,i:i+square_size, j:j+square_size,:] = 0
        probabilities = model.predict(x_copy, steps=1, verbose=1)
        print(probabilities)
        print(y_test[0])
        probabilities = np.squeeze(probabilities)
        #p = np.where(np.squeeze(y_test[0]) == 1)[0][0]
        heatmap[i/2,j/2] = probabilities
heatmap = np.resize(heatmap, (width,height))
#np.save(output_path+'_heatmap.npy', heatmap)
#np.save(output_path+'_image.npy', x[0].astype(int))
fig = plt.figure()
plt.axis('off')
plt.imshow(x_test[0].astype(int), interpolation='nearest')
print(heatmap.shape)
plt.imshow(heatmap, cmap='hot', interpolation='nearest', alpha=.5)
plt.show
